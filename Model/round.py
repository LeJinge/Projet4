    ...
    def to_dict(self):
        try:
            ...
            if len(self.matchs) > 0:
                round_data["matchs"] = [match.to_dict() for match in self.matchs]
            return round_data

        except AttributeError as e:
            raise ValueError(f"Failed to serialize the Round object: {e}")
    ...
    def from_dict(cls, data):
        try:
            ...
            if "name" not in data or "date_debut" not in data:
                raise ValueError("Missing necessary fields for deserialization.")

            # Créer une instance de Round
            round_obj = cls()
            round_obj.name = data["name"]
            round_obj.date_debut = datetime.strptime(data["date_debut"], "%Y-%m-%d %H:%M:%S")

            # Vérifier si la date de fin est présente et la convertir
            if data.get("date_fin"):
                round_obj.date_fin = datetime.strptime(data["date_fin"], "%Y-%m-%d %H:%M:%S")
            else:
            ...
            if not isinstance(match_data, list):
                raise ValueError(f"Expected 'matchs' to be a list, got {type(match_data).__name__} instead.")
