...
class Tournament:
    ...
    def from_dict(cls, data):
        if not all(
                key in data for key in [
                    "name", "place", "start_date", "end_date", "rounds_numbers"
                ]):
        ...
        tournament.list_tours = [
            Round.from_dict(tour_data)
            for tour_data in data.get("list_tours", [])
        ]
        tournament.list_player_save = [
            Player.from_dict(player_data)
            for player_data in data.get("list_player_save", [])
        ]
    ...
    def to_dict(self):
        return {
            ...
            "list_tours": [
                round_obj.to_dict() for round_obj in self.list_tours
            ],
            "list_player_save": [
                player.to_dict() for player in self.list_player_save
            ],
